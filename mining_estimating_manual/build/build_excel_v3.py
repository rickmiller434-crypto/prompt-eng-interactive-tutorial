...
Manual & Rate Library (RMM-CIVIL-CANADA-2026-MANUAL-REV3).
...
OUT = os.path.join(os.path.dirname(__file__), "..", "RMM-CIVIL-CANADA-2026-MANUAL-REV3.xlsx")
...
SUBTITLE = ("Rev 3 — End-to-End Mining Heavy Civil | Consolidated SoR + Real Market Benchmark Cross-Reference | "
...
sor_rows = [
    ...
    ("I — BLASTING (FULL SCOPE)","B-01-002","Drill blast hole 89-115mm — rotary drill",                 "m",  0.25, None, None, None, None, None, None,    25,    45, "10%", "0%",  "D1", "Rate per lineal metre of hole"),
    ...
    ("I — BLASTING (FULL SCOPE)","B-01-004","Controlled blasting near structures — presplit",           "m",  0.80, None, None, None, None, None, None,    55,    95, "10%", "0%",  "D1", "Presplit perimeter holes"),
    ...
    ("O — ROCK ANCHORS & STABILIZATION","B-07-002","Rock anchor active tensioned 32mm strand",           "ea", 8.00, None, None, None, None, None, None,  2500,  5500, "10%", "0%",  "D1", "Pre-stressed; headplate; torqued to spec"),
...
def _decompose_rev3(rows):
    """For each row where Unit MH (idx 4) and crew code (idx 15) are set and
    Unit Labour / Unit Equip (idx 6, 7) are None, compute and fill them from
    CREW_RATES.  Default productivity to 0.91 if Prod (idx 5) is None.
    Leave Unit Material (idx 8) / Subcontract (idx 9) alone — they remain as
    None for source items where supplier qty was bundled into Total Low/High."""
    out = []
    for r in rows:
        r = list(r)
        ...
        if (u_lab is None and u_eq is None and mh and crew in CREW_RATES):
            ...
            r[6] = round(effective_mh * lab_hr, 2)
            r[7] = round(effective_mh * eq_hr, 2)
            # CDI default if None for these items
            if r[10] is None:
                r[10] = 0.06
        out.append(tuple(r))
    return out

sor_rows = _decompose_rev3(sor_rows)
